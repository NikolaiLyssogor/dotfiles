...
-- Returns current JAVA_HOME by running java in $HOME
local function get_java_home()
  local home = vim.uv.os_homedir()
  -- Construct the shell command to ensure pwd = $HOME
  local cmd = "cd " ..
  home .. " && java -XshowSettings:properties -version 2>&1 | grep 'java.home =' | awk -F'= ' '{print $2}'"
  local java_home = vim.fn.system(cmd):gsub('%s+$', '')
  if java_home == '' then
    error("Unable to determine JAVA_HOME")
  end
  return java_home
end
